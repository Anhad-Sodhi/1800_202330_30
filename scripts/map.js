...
function addEventPins(map, features) {
  // Add event pin icon
  map.loadImage(
...
function showMap() {
  navigator.geolocation.getCurrentPosition((position) => {
    ...
    initMapbox();
    const map = createMap("map", "mapbox://styles/dand5/clofy6cum004j01qg3xph52ua", userLocation, 14);

    map.on("load", () => {
      fetchEventData((allEvents) => {
        const features = createEventFeatures(allEvents);
        addEventPins(map, features);
      });
    });
...
function showListingMap() {
  navigator.geolocation.getCurrentPosition((position) => {
    const userLocation = [position.coords.longitude, position.coords.latitude];
    initMapbox();
    const map = createMap("map", "mapbox://styles/dand5/clofy6cum004j01qg3xph52ua", userLocation, 17);
