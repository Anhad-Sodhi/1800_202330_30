...
async function doAll() {
    ...
    snapshot.forEach(doc => {
        ...
        db.collection("users").doc(firebase.auth().currentUser.uid).get().then(test => {
            ...
            if (!ownPost) {
                // Populate the page with listings
                addListings(count, doc);
...
// Add listings that are not the user's
function addListings(count, doc) {
    // Create a listing div
    var listing = document.createElement("div");
    listing.setAttribute("class", "list " + doc.id);
